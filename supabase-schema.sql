...
-- שיתוף לפי מייל (ללא קישור): מי משויך לאיזו רשימה
create table if not exists list_participants (
  list_id text not null,
  participant_email text not null,
  added_by_email text,
  created_at timestamptz not null default now(),
  primary key (list_id, participant_email)
);
...
drop policy if exists "משתמשים משתתפים" on list_participants;
create policy "משתמשים משתתפים"
  on list_participants for all
  to authenticated
  using (true)
  with check (true);
...
alter publication supabase_realtime add table list_participants;
